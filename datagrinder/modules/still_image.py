...
class StillImage(Asset):
    ...
    base_args = [config['datagrinder']['shell_bin'], '-c']

    gmagick_cmd_head = config['datagrinder']['gmagick_bin'] + ' convert'

    gmagick_args = ''

    gmagick_cmd_tail = '- -' # Pipe in and out

    exiftool_cmd_head = config['datagrinder']['exiftool_bin']
    ...
    def _action_metadata(self, preset):
        '''Manipulate embedded metadata fields according to a preset.'''
        ...
        remove_args = '-' + '= -'.join(settings['remove']) + '= ' \
                if settings['remove'] else ''
        preserve_args = '--' + ' --'.join(settings['preserve']) \
                if settings['preserve'] else ''
    ...
    def _apply_actions(self, actions):
        ''' Loop over single actions and apply them in a prefixed order. '''

        self.gmagick_args, self.exiftool_args, \
                gmagick_cmd, exiftool_cmd, args = '','','','',''

        # Gather arguments.
        for action in actions.keys():
            if not actions[action] == False:
                cherrypy.log('Setting option \'{}\' to \'{}\'.'.format(
                        action, actions[action]))
                eval('self._action_{}(\'{}\')'.format(
                        action, actions[action]))

        if self.gmagick_args:
            gmagick_cmd = '{} {} {}'.format(self.gmagick_cmd_head,
                    self.gmagick_args, self.gmagick_cmd_tail)

        if self.exiftool_args:
            exiftool_cmd = '{} {} {}'.format(self.exiftool_cmd_head,
                    self.exiftool_args, self.exiftool_cmd_tail)
